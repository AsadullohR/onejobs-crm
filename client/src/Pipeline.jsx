...
useEffect(() => {
  ...
  const doAddStage=()=>{
    const lbl=newStageLabel.trim(); if(!lbl)return;
    const key=lbl.replace(/[^a-zA-Z0-9 ']/g,"").trim();
    if(stages.some(s=>s.key===key)){alert("Bu nom allaqachon mavjud");return;}
    setStages(p=>[...p,{key:lbl,label:lbl,c:newStageColor}]);
    setNewStageLabel(""); setNewStageColor("#6366f1");
  };
  ...
  const inpS=inp(T);
  const exportPipelineCSV=()=>{const cols=["ID","Ism","Tel","Holat","Mamlakat","Manba","Masul","Izoh"];const rows=flt.map(l=>[l.id,l.name||"",l.phone||"",l.status||"",l.country||"",l.source||"",l.owner||"",String(l.comment||"").replace(/,/g," ")].join(","));const txt=cols.join(",")+String.fromCharCode(10)+rows.join(String.fromCharCode(10));const blob=new Blob([txt],{type:"text/csv"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="pipeline.csv";a.click();};

  const moveUp=(idx)=>{if(idx===0)return;const a=[...stages];[a[idx-1],a[idx]]=[a[idx],a[idx-1]];setStages(a);};
  const moveDown=(idx)=>{if(idx===stages.length-1)return;const a=[...stages];[a[idx+1],a[idx]]=[a[idx],a[idx+1]];setStages(a);};
  const onDragStart=(e,key)=>{setDragStg(key);e.dataTransfer.effectAllowed="move";};
  const onDragOver=(e,key)=>{e.preventDefault();setDragOverStg(key);};
  const onDrop=(e,targetKey)=>{
    e.preventDefault();
    if(!dragStg||dragStg===targetKey)return;
    const arr=[...stages];
    const fi=arr.findIndex(s=>s.key===dragStg); const ti=arr.findIndex(s=>s.key===targetKey);
    const [m]=arr.splice(fi,1); arr.splice(ti,0,m);
    setStages(arr); setDragStg(null); setDragOverStg(null);
  };
  ...
  return <div>
    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,flexWrap:"wrap"}}>
      ...
      <div style={{marginLeft:"auto",display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
        <div style={{position:"relative"}}><span style={{position:"absolute",left:7,top:"50%",transform:"translateY(-50%)",color:T.muted}}>{I.search}</span>
          <input ref={searchRef} value={search} onChange={e=>setSearch(e.target.value)} placeholder="Qidirish... (Ctrl+A)" style={{...inpS,paddingLeft:23,width:150,fontSize:11}}/></div>
    ...
    {editMode&&<div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:9,padding:"12px 14px",marginBottom:10}}>
      <div style={{fontSize:11,fontWeight:700,color:T.text,marginBottom:10}}>⚙️ Bosqichlarni boshqarish</div>
      <div style={{maxHeight:260,overflowY:"auto",marginBottom:10}}>
        {stages.map((stage,idx)=>(
          <div key={stage.key} draggable onDragStart={e=>onDragStart(e,stage.key)} onDragOver={e=>onDragOver(e,stage.key)} onDrop={e=>onDrop(e,stage.key)}
            style={{display:"flex",alignItems:"center",gap:6,padding:"5px 8px",marginBottom:3,background:dragOverStg===stage.key?`${T.accent}18`:T.card2,border:`1px solid ${dragOverStg===stage.key?T.accent:stage.c+"33"}`,borderRadius:6,cursor:"grab",borderLeft:`3px solid ${stage.c}`}}>
            <span style={{color:T.muted,fontSize:11}}>⠿</span>
            <span style={{width:10,height:10,borderRadius:"50%",background:stage.c,flexShrink:0}}/>
            {editingStage===stage.key
              ? <input value={stageEdit.label||stage.label} onChange={e=>setStageEdit(p=>({...p,label:e.target.value}))} style={{...inpS,flex:1,padding:"2px 6px",fontSize:10}} autoFocus onKeyDown={e=>{if(e.key==="Enter"){setStages(p=>p.map(s=>s.key===stage.key?{...s,label:stageEdit.label||s.label,c:stageEdit.c||s.c}:s));setEditingStage(null);}if(e.key==="Escape")setEditingStage(null);}}/>
              : <span style={{flex:1,fontSize:10,color:T.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{stage.label}</span>
            }
            {editingStage===stage.key&&<div style={{display:"flex",gap:2}}>{["#6366f1","#f59e0b","#22c55e","#ec4899","#3b82f6","#0891b2","#f97316","#ef4444","#8b5cf6","#10b981"].map(c=><div key={c} onClick={()=>setStageEdit(p=>({...p,c}))} style={{width:12,height:12,borderRadius:"50%",background:c,cursor:"pointer",border:`2px solid ${(stageEdit.c||stage.c)===c?T.text:"transparent"}`}}/>)}</div>}
            <span style={{fontSize:8,color:T.muted,flexShrink:0}}>({grp[stage.key]?.length||0})</span>
            <button onClick={()=>moveUp(idx)} disabled={idx===0} style={{padding:"1px 4px",background:"none",border:"none",cursor:"pointer",color:T.muted,fontSize:10,opacity:idx===0?0.2:1}}>↑</button>
            <button onClick={()=>moveDown(idx)} disabled={idx===stages.length-1} style={{padding:"1px 4px",background:"none",border:"none",cursor:"pointer",color:T.muted,fontSize:10,opacity:idx===stages.length-1?0.2:1}}>↓</button>
            {editingStage===stage.key
              ? <button onClick={()=>{setStages(p=>p.map(s=>s.key===stage.key?{...s,label:stageEdit.label||s.label,c:stageEdit.c||s.c}:s));setEditingStage(null);}} style={{padding:"2px 6px",borderRadius:4,background:T.accent,color:"#fff",border:"none",cursor:"pointer",fontSize:9,fontWeight:700}}>✓</button>
              : <button onClick={()=>{setEditingStage(stage.key);setStageEdit({label:stage.label,c:stage.c});}} style={{padding:"2px 6px",borderRadius:4,background:`${T.accent}22`,color:T.accent,border:`1px solid ${T.accent}44`,cursor:"pointer",fontSize:9}}>{I.edit}</button>
            }
            <button onClick={()=>{const dcnt=grp[stage.key]?.length||0;if(dcnt>0){alert('Bu bosqichda '+dcnt+' ta mijoz bor. Avval ularni otkazing.');return;}if(window.confirm(stage.label+' ochirilsinmi?'))setStages(p=>p.filter(s=>s.key!==stage.key));}} style={{padding:'2px 6px',borderRadius:4,background:`${T.red}22`,color:T.red,border:`1px solid ${T.red}44`,cursor:'pointer',fontSize:9}}>{I.trash}</button>
          </div>
        ))}
      </div>
      {/* Add new stage */}
      <div style={{display:"flex",gap:7,alignItems:"center",padding:"8px 10px",background:`${T.accent}08`,borderRadius:7,border:`1px dashed ${T.accent}44`}}>
        <input value={newStageLabel} onChange={e=>setNewStageLabel(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doAddStage()} placeholder="Yangi bosqich nomi..." style={{...inpS,flex:1,fontSize:10}} />
        <div style={{display:"flex",gap:3}}>
          {["#6366f1","#f59e0b","#22c55e","#ec4899","#3b82f6","#0891b2","#f97316","#ef4444"].map(c=><div key={c} onClick={()=>setNewStageColor(c)} style={{width:14,height:14,borderRadius:"50%",background:c,cursor:"pointer",border:`2px solid ${newStageColor===c?T.text:"transparent"}`}}/>)}
        </div>
        <button onClick={doAddStage} style={{padding:"5px 12px",borderRadius:6,background:T.accent,color:"#fff",border:"none",cursor:"pointer",fontSize:10,fontWeight:700,flexShrink:0}}>+ Qo'shish</button>
      </div>
    </div>}
    <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:12,alignItems:"flex-start"}}>
      {stages.map(stage=>{
        const cards=grp[stage.key]||[];
        return <div key={stage.key} draggable={editMode} onDragStart={e=>editMode&&onDragStart(e,stage.key)} onDragOver={e=>editMode&&onDragOver(e,stage.key)} onDrop={e=>editMode&&onDrop(e,stage.key)}
          style={{minWidth:210,maxWidth:210,flexShrink:0,opacity:dragStg===stage.key?0.5:1,border:editMode&&dragOverStg===stage.key?`2px dashed ${T.accent}`:"none",borderRadius:9}}>
          <div style={{background:`${stage.c}${T.dark?"22":"15"}`,border:`1px solid ${stage.c}44`,borderRadius:"8px 8px 0 0",padding:"6px 8px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
