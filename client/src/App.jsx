...
function Pipeline({leads, tasks, team, user, open, addLead, config, roles}) {
  ...
  return <div>
    ...
    {showFilters&&<div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:9,padding:"10px 12px",marginBottom:10,display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
      ...
      <button onClick={()=>{setFOwner(0);setFCountry("");setFPosition("");setFDate("");setFHasTasks(false);}} style={{padding:"4px 9px",borderRadius:5,background:`${T.red}22`,color:T.red,border:`1px solid ${T.red}44`,cursor:"pointer",fontSize:10,fontWeight:600}}>Tozalash</button>
    </div>}
...
function LeadsList({leads, tasks, team, user, open, addLead, config, roles, setLeads}) {
  const T=useT();
  const perm=roles[user.role]||{};
  const [s,setS]=useState(""); const [fSt,setFSt]=useState(""); const [fC,setFC]=useState(""); const [fO,setFO]=useState(0); const [fSec,setFSec]=useState(""); const [fDate,setFDate]=useState("");
  const vis=useMemo(()=>leads.filter(l=>{
    if(s&&!l.name.toLowerCase().includes(s.toLowerCase())&&!l.phone?.includes(s)&&!l.id.toLowerCase().includes(s.toLowerCase()))return false;
    if(fSt&&l.status!==fSt)return false;
    if(fC&&l.country!==fC)return false;
    if(fO&&l.ownerSales!==fO&&l.ownerConsult!==fO&&l.ownerDocs!==fO)return false;
    if(fSec&&l.sector!==fSec)return false;
    if(fDate&&!dateRange(l.createdAt,fDate))return false;
    return true;
  }),[leads,s,fSt,fC,fO,fSec,fDate]);
  const inpS=inp(T);
  return <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
      <div><h1 style={{fontSize:18,fontWeight:900,color:T.text,margin:0}}>Mijozlar</h1><p style={{color:T.muted,margin:"1px 0 0",fontSize:10}}>{vis.length} ta</p></div>
      {perm.canEdit&&<button onClick={addLead} style={{display:"flex",alignItems:"center",gap:4,padding:"7px 12px",borderRadius:7,background:T.accent,color:"#fff",fontWeight:700,fontSize:11,border:"none",cursor:"pointer"}}>{I.plus} Yangi</button>}
    </div>
    <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:9,padding:"9px 11px",marginBottom:9}}>
      <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr 1fr",gap:6,marginBottom:6}}>
        <div style={{position:"relative"}}><span style={{position:"absolute",left:7,top:"50%",transform:"translateY(-50%)",color:T.muted}}>{I.search}</span><input value={s} onChange={e=>setS(e.target.value)} placeholder="ID, Ism, Telefon..." style={{...inpS,paddingLeft:22}}/></div>
        <select value={fSt} onChange={e=>setFSt(e.target.value)} style={inpS}><option value="">Holat</option>{STAGES.map(x=><option key={x.key} value={x.key}>{x.label}</option>)}</select>
        <select value={fC} onChange={e=>setFC(e.target.value)} style={inpS}><option value="">Mamlakat</option>{config.countries.map(c=><option key={c}>{c}</option>)}</select>
        <select value={fSec} onChange={e=>setFSec(e.target.value)} style={inpS}><option value="">Soha</option>{config.sectors.map(x=><option key={x}>{x}</option>)}</select>
        <select value={fO} onChange={e=>setFO(Number(e.target.value))} style={inpS}><option value={0}>Mas'ul</option>{team.filter(t=>t.role!=="partner").map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select>
        <select value={fDate} onChange={e=>setFDate(e.target.value)} style={inpS}><option value="">Vaqt</option><option value="today">Bugun</option><option value="week">Bu hafta</option><option value="month">Bu oy</option></select>
      </div>
      <button onClick={()=>{setS("");setFSt("");setFC("");setFO(0);setFSec("");setFDate("");}} style={{padding:"4px 9px",borderRadius:5,background:`${T.red}22`,color:T.red,border:`1px solid ${T.red}44`,cursor:"pointer",fontSize:10,fontWeight:600}}>Tozalash</button>
    </div>
    <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:9,overflow:"auto"}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
        <thead><tr style={{borderBottom:`1px solid ${T.border}`,background:T.card2}}>
          {["№","ID","Ism","Tel","Mamlakat","Lavozim","Mas'ullar","KPI","Holat","To'lov",""].map(h=><th key={h} style={{color:T.muted,fontWeight:600,textAlign:"left",padding:"8px 9px",fontSize:9,textTransform:"uppercase"}}>{h}</th>)}
        </tr></thead>
        <tbody>
          {vis.map((l,i)=>{
            ...
            return <tr key={l.id} style={{borderBottom:`1px solid ${T.border}22`,background:gone?`${T.green}08`:lost?`${T.red}08`:"transparent",cursor:"pointer"}} onClick={()=>open(l)}>
              <td style={{padding:"7px 9px",color:T.muted}}>{i+1}</td>
              <td style={{padding:"7px 9px"}}><span style={{color:T.accent,fontWeight:700}}>{l.id}</span></td>
              <td style={{padding:"7px 9px"}}><div style={{display:"flex",alignItems:"center",gap:4}}><span style={{color:T.text,fontWeight:600}}>{l.name||"–"}</span>{od&&<span style={{color:T.red,fontSize:9}}>⚠️</span>}</div></td>
              <td style={{padding:"7px 9px",color:T.sub,fontSize:10}}>{l.phone}</td>
              <td style={{padding:"7px 9px",color:T.sub}}>{l.country||"–"}</td>
              <td style={{padding:"7px 9px",color:T.sub,maxWidth:90,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{l.position||"–"}</td>
              <td style={{padding:"7px 9px"}}><div style={{display:"flex",gap:2}}>{owners.map(id=><Av key={id} id={id} team={team} size={18}/>)}</div></td>
              <td style={{padding:"7px 9px"}}>
                <div style={{display:"flex",gap:2}}>
                  {[["kpiSales","K₁",T.green],["kpiConsult","K₂",T.blue],["kpiDocs","K₃",T.cyan]].map(([k,lb,c])=>(
                    <span key={k} style={{fontSize:8,background:l[k]?`${c}22`:`${T.border}22`,color:l[k]?c:T.border,borderRadius:3,padding:"0 3px",fontWeight:700,border:`1px solid ${l[k]?c+"44":T.border}`}}>{lb}</span>
                  ))}
                </div>
              </td>
              ...
              <td style={{padding:"7px 9px"}}>
                <div style={{display:"flex",gap:2,alignItems:"center"}}>
                  {[["q1","#ec4899"],["q2","#8b5cf6"],["q3","#3b82f6"]].map(([k,c])=>(
                    <div key={k} style={{width:13,height:13,borderRadius:2,border:`1px solid ${l[k]?c+"88":T.border}`,background:l[k]?`${c}22`:"transparent",display:"flex",alignItems:"center",justifyContent:"center",fontSize:7,color:l[k]?c:T.border}}>
                      {l[k]&&"✓"}{l[k]&&l[k+"R"]&&<span style={{position:"absolute",bottom:-2,right:-2,width:4,height:4,borderRadius:"50%",background:T.green}}/>}
                    </div>
                  ))}
                  {l.xba&&<span style={{fontSize:7,background:`${T.yellow}22`,color:T.yellow,border:`1px solid ${T.yellow}44`,borderRadius:2,padding:"0 2px",fontWeight:700}}>XBA</span>}
                </div>
              </td>
              <td style={{padding:"7px 9px"}}>{perm.canEdit&&<button onClick={e=>{e.stopPropagation();open(l);}} style={{padding:"2px 5px",borderRadius:4,background:`${T.accent}22`,color:T.accent,border:`1px solid ${T.accent}44`,cursor:"pointer",fontSize:9}}>{I.edit}</button>}</td>
            </tr>;
          })}
        </tbody>
      </table>
      {vis.length===0&&<div style={{color:T.muted,textAlign:"center",padding:30,fontSize:11}}>Mijoz topilmadi</div>}
    </div>
  </div>;
}
...
function Dashboard({leads, tasks, user, team, txns, roles}) {
  const T=useT();
  const total=leads.length; const gone=leads.filter(l=>DONE.includes(l.status)).length;
  ...
  const byCon=Object.entries(leads.reduce((m,l)=>{if(l.country){m[l.country]=(m[l.country]||0)+1;}return m;},{})).sort(([,a],[,b])=>b-a).slice(0,6);
  const bySrc=Object.entries(leads.reduce((m,l)=>{if(l.source){m[l.source]=(m[l.source]||0)+1;}return m;},{})).sort(([,a],[,b])=>b-a).slice(0,8);
  const maxC=Math.max(...byCon.map(([,n])=>n),1); const maxS=Math.max(...bySrc.map(([,n])=>n),1);
  const teamStats=team.filter(t=>["sales","manager","docs"].includes(t.role)).map(t=>({t,total:leads.filter(l=>l.ownerSales===t.id||l.ownerConsult===t.id||l.ownerDocs===t.id).length,gone:leads.filter(l=>(l.ownerSales===t.id||l.ownerConsult===t.id||l.ownerDocs===t.id)&&DONE.includes(l.status)).length,kpi:leads.filter(l=>(l.ownerSales===t.id&&l.kpiSales)||(l.ownerConsult===t.id&&l.kpiConsult)||(l.ownerDocs===t.id&&l.kpiDocs)).length})).filter(x=>x.total>0).sort((a,b)=>b.gone-a.gone);
  const funnelGroups=[["Yangi",["Yangi","Qilindi"]],["Aloqa",["Bog'landi","Onlayn Suhbat Uchun","Onlayn Suhbat","Suhbat"]],["Bitim",["Shartnoma qildi","XBA To'lov qildi","CV Topshirildi","Interview ga qo'yildi","Ishga qabul qilindi"]],["To'lov",["1 - Qism To'landi"]],["Hujjatlar",["Hujjatlar Tayyorlanmoqda","Hujjatlar Jonatilishga Tayyor","Hujjatlar Jonatildi","Ish shartnomasi keldi","Ish shartnomasi imzolandi","Taklifnoma keldi"]],["Viza",["Elchixonaga Hujjatlar Tayyor","Vizaga Topshirildi","Viza Oldi"]],["Jo'nab ketdi",["Jo'nab ketdi"]]];
  const fData=funnelGroups.map(([g,stages])=>({g,n:leads.filter(l=>stages.includes(l.status)).length})); const maxF=Math.max(...fData.map(x=>x.n),1);
  ...
  return <div>
    <div style={{marginBottom:12}}><h1 style={{fontSize:18,fontWeight:900,color:T.text,margin:0}}>Super Dashboard</h1></div>
    <div style={{display:"grid",gridTemplateColumns:`repeat(${perm.canFin?5:4},1fr)`,gap:10,marginBottom:12}}>
      <StatCard icon="👥" label="Jami Mijozlar" value={total} sub={`${gone} muvaffaqiyatli`}/>
      <StatCard icon="✈️" label="Jo'nab Ketdi" value={gone} sub={`${total?((gone/total)*100).toFixed(0):0}%`} color={T.green}/>
      <StatCard icon="📋" label="Jarayonda" value={leads.filter(l=>![...DONE,...LOST].includes(l.status)).length} color={T.purple}/>
      <StatCard icon="⚠️" label="Vazifalarim" value={myT.length} sub={`${myT.filter(t=>isOD(t.due)).length} o'tgan`} color={T.yellow}/>
      {perm.canFin&&<div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:10,padding:"12px 14px",borderTop:`3px solid ${T.yellow}`}}>
        <div style={{fontSize:10,color:T.muted,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.05em"}}>💰 Moliya</div>
        <div style={{fontSize:18,fontWeight:900,color:T.yellow}}>{fmtMs(totalInc-totalExp)}</div>
        <div style={{fontSize:9,color:T.muted}}>Sof: {fmtMs(sofF)} so'm</div>
      </div>}
    </div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:12}}>
      {/* Funnel */}
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:10,padding:14}}>
        <h3 style={{margin:"0 0 10px",fontSize:11,fontWeight:700,color:T.text,textTransform:"uppercase",letterSpacing:"0.06em"}}>🔽 Pipeline</h3>
        {fData.map(({g,n},i)=>(
          <div key={g} style={{marginBottom:6}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:2}}><span style={{fontSize:10,color:T.sub}}>{g}</span><span style={{fontSize:10,fontWeight:700,color:T.text}}>{n}</span></div>
            <div style={{background:T.border,borderRadius:3,height:4}}><div style={{width:`${(n/maxF)*100}%`,background:FC[i],borderRadius:3,height:4}}/></div>
          </div>
        ))}
      </div>
      {/* Team */}
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:10,padding:14}}>
        <h3 style={{margin:"0 0 10px",fontSize:11,fontWeight:700,color:T.text,textTransform:"uppercase",letterSpacing:"0.06em"}}>👔 Xodimlar</h3>
        {teamStats.map(({t,total:tot,gone:g,kpi})=>(
          <div key={t.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderBottom:`1px solid ${T.border}22`}}>
            <Av id={t.id} team={team} size={22}/>
            <div style={{flex:1}}><div style={{fontSize:11,fontWeight:700,color:T.text}}>{t.name}</div><div style={{fontSize:8,color:T.muted}}>Jami:{tot} · Jo'nab:{g} · KPI:{kpi}</div></div>
            {perm.canFin&&<span style={{fontSize:9,fontWeight:700,color:T.green}}>{fmtMs(txns.filter(x=>x.type==="income"&&leads.find(l=>l.id===x.leadId&&(l.ownerSales===t.id||l.ownerConsult===t.id||l.ownerDocs===t.id))).reduce((s,x)=>s+x.amount,0))}</span>}
          </div>
        ))}
      </div>
      {/* Country */}
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:10,padding:14}}>
        <h3 style={{margin:"0 0 10px",fontSize:11,fontWeight:700,color:T.text,textTransform:"uppercase",letterSpacing:"0.06em"}}>🌍 Mamlakatlar</h3>
        {byCon.map(([c,n])=>(
          <div key={c} style={{display:"flex",alignItems:"center",gap:7,marginBottom:6}}>
            <span style={{color:T.sub,fontSize:10,width:75,flexShrink:0}}>{c}</span>
            <div style={{flex:1,background:T.border,borderRadius:3,height:4}}><div style={{width:`${(n/maxC)*100}%`,background:T.accent,borderRadius:3,height:4}}/></div>
            <span style={{color:T.text,fontSize:10,fontWeight:700,width:16,textAlign:"right"}}>{n}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:10,padding:14}}>
        <h3 style={{margin:"0 0 10px",fontSize:11,fontWeight:700,color:T.text,textTransform:"uppercase",letterSpacing:"0.06em"}}>📣 Manba (Source)</h3>
        {bySrc.map(([src,n])=>(
          <div key={src} style={{display:"flex",alignItems:"center",gap:7,marginBottom:6}}>
            <span style={{color:T.sub,fontSize:10,width:80,flexShrink:0}}>{src}</span>
            <div style={{flex:1,background:T.border,borderRadius:3,height:5}}><div style={{width:`${(n/maxS)*100}%`,background:T.cyan,borderRadius:3,height:5}}/></div>
            <span style={{color:T.text,fontSize:10,fontWeight:700,width:20,textAlign:"right"}}>{n}</span>
            <span style={{fontSize:8,color:T.muted,width:30,textAlign:"right"}}>{total?`${((n/total)*100).toFixed(0)}%`:"–"}</span>
          </div>
        ))}
      </div>
      {/* Finance chart */}
      {perm.canFin&&<div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:10,padding:14}}>
        ...
        <div style={{display:"flex",gap:10,alignItems:"flex-end",height:80,paddingBottom:16,position:"relative"}}>
          {months.map((m,i)=>(
            <div key={m} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
              <div style={{width:"100%",display:"flex",gap:1,alignItems:"flex-end",height:65}}>
                <div style={{flex:1,background:`${T.green}88`,borderRadius:"3px 3px 0 0",height:`${(mInc[i]/maxB)*65}px`,minHeight:2}}/>
                <div style={{flex:1,background:`${T.red}88`,borderRadius:"3px 3px 0 0",height:`${(mExp[i]/maxB)*65}px`,minHeight:2}}/>
              </div>
              <span style={{fontSize:8,color:T.muted,position:"absolute",bottom:0}}>{"JFMAM"[i]}</span>
            </div>
          ))}
        </div>
        <div style={{display:"flex",gap:12,justifyContent:"center"}}>
          {[[T.green,"Kirim"],[T.red,"Chiqim"]].map(([c,l])=><div key={l} style={{display:"flex",alignItems:"center",gap:3,fontSize:9,color:T.muted}}><div style={{width:7,height:7,borderRadius:2,background:c}}/>{l}</div>)}
        </div>
      </div>}
    </div>
    {/* Due tasks */}
    <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:10,padding:14}}>
      ...
      {tasks.filter(t=>t.status!=="done"&&(isOD(t.due)||isSoon(t.due))).length===0&&<div style={{color:T.muted,fontSize:11,textAlign:"center",padding:10}}>Muddati yaqin vazifa yo'q ✅</div>}
    </div>
  </div>;
}
...
function Sidebar({user, pg, go, logout, notif, roles, dark, setDark, col, setCol}) {
  ...
  const allowed={admin:NAV.map(n=>n.k),manager:["dashboard","pipeline","leads","tasks","visa","team","settings","empreport"],sales:["dashboard","pipeline","leads","tasks"],docs:["dashboard","leads","tasks"],partner:["leads"]};
  const perm=roles[user.role]||{};
  let vis=NAV.filter(n=>(allowed[user.role]||[]).includes(n.k));
  if(!perm.canFin){vis=vis.filter(n=>n.k!=="finance"&&n.k!=="salary"&&n.k!=="empreport");}
  const W=col?54:196;
  ...
  return <ThemeCtx.Provider value={T}>
    <div style={{display:"flex",minHeight:"100vh",background:T.bg,fontFamily:"'Segoe UI',system-ui,sans-serif",color:T.text}}>
      ...
      <div style={{flex:1,overflow:"auto",minWidth:0,display:"flex",flexDirection:"column"}}>
        ...
        <div style={{flex:1,padding:page==="finance"?0:"14px 18px",overflow:page==="finance"?"hidden":"auto"}}>
