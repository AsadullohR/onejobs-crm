    ...
    <div style={{width:530,background:T.bg,borderLeft:`1px solid ${T.border}`,display:"flex",flexDirection:"column",boxShadow:T.shadow}}>
      ...
      <div style={{padding:"14px 16px",flex:1,overflowY:"auto"}}>
        ...
        {tab==="info"&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
          ...
          <div><label style={labS}>Manba</label><select value={form.source||""} onChange={e=>f("source",e.target.value)} disabled={!canEdit&&!isNew} style={inpS}>
            <option value="">–</option>
            {(()=>{const partnerNames=team.filter(t=>t.role==="partner").map(t=>t.name);return <>
              {form.source&&!config.sources.includes(form.source)&&!partnerNames.includes(form.source)&&<option value={form.source}>{form.source}</option>}
              {partnerNames.length>0&&<optgroup label="🤝 Hamkorlar">{partnerNames.map(n=><option key={n} value={n}>{n}</option>)}</optgroup>}
              <optgroup label="Manbalar">{config.sources.map(s=><option key={s}>{s}</option>)}</optgroup>
            </>;})()}
          </select></div>
