    ...
    <div style={{width:530,background:T.bg,borderLeft:`1px solid ${T.border}`,display:"flex",flexDirection:"column",boxShadow:T.shadow}}>
      ...
      <div style={{padding:"14px 16px",flex:1,overflowY:"auto"}}>
        ...
        {tab==="info"&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
          ...
          <div><label style={labS}>Manba</label><select value={form.source||""} onChange={e=>f("source",e.target.value)} disabled={!canEdit&&!isNew} style={inpS}><option value="">–</option>{form.source&&!config.sources.includes(form.source)&&<option value={form.source}>🤝 {form.source}</option>}{config.sources.map(s=><option key={s}>{s}</option>)}</select></div>
