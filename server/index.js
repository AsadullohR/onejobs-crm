  ...
  try {
    const query = `
      SELECT
        ...
        (l.q1_receipt IS NOT NULL) AS q1_receipt, (l.q2_receipt IS NOT NULL) AS q2_receipt,
        (l.q3_receipt IS NOT NULL) AS q3_receipt, (l.xba_receipt IS NOT NULL) AS xba_receipt,
...
// ─── FUNNEL STATS (cohort, reached-stage-or-beyond) ──────────────────────────
// Cohort = leads CREATED in the selected period. Each stage counts how many
// of that cohort EVER reached that stage or any later one — evidence comes
...
const REACH_STAGES = [
  ...
  { key: "Ishga qabul qilindi",  statuses: ["Ishga qabul qilindi"] },
  { key: "1-qism to'ladi",       date: "l.q1_date::text", bool: "l.q1", statuses: ["1 - Qism To'landi"] },
  { key: "Hujjatlar jo'natildi", statuses: ["Hujjatlar Jonatildi", "Ish shartnomasi keldi", "Ish shartnomasi imzolandi"] },
  { key: "Taklifnoma keldi",     statuses: ["Taklifnoma keldi"] },
  { key: "Elchixonaga tayyor",   statuses: ["Elchixonaga Hujjatlar Tayyor"] },
  { key: "Vizaga topshirdi",     statuses: ["Vizaga Topshirildi"] },
  { key: "Viza oldi",            statuses: ["Viza Oldi", "Jo'nab ketdi"] },
];
...
app.get("/api/stats/funnel", auth, async (req, res) => {
  ...
  try {
    ...
    // e{i}: evidence the lead ever entered stage i
    const evidence = REACH_STAGES.map((st, i) => {
      params.push(st.statuses);
      const p = params.length;
      const parts = [];
      if (st.date) parts.push(`${st.date} IS NOT NULL`);
      if (st.bool) parts.push(`${st.bool} = true`);
      parts.push(`l.status = ANY($${p})`);
      parts.push(`EXISTS (SELECT 1 FROM status_log sl WHERE sl.lead_id = l.id AND sl.status = ANY($${p}))`);
      return `(${parts.join(" OR ")}) AS e${i}`;
    });
    // reached stage i ⇔ evidence for stage i OR any later stage
    const reachCols = REACH_STAGES.map((_, i) =>
      `COUNT(*) FILTER (WHERE ${REACH_STAGES.map((_, j) => `e${j}`).slice(i).join(" OR ")}) AS r${i}`);
    const { rows } = await pool.query(
      `WITH ev AS (
       ...
       SELECT COUNT(*) AS total, ${reachCols.join(", ")} FROM ev`,
      params,
    );
    const r = rows[0];
    res.json({
      ...
      stages: [
        { key: "Jami leadlar", n: Number(r.total) },
        ...REACH_STAGES.map((st, i) => ({ key: st.key, n: Number(r[`r${i}`]) })),
      ],
    });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.listen(PORT, async () => {
  ...
  try {
    ...
    await pool.query(`ALTER TABLE leads ADD COLUMN IF NOT EXISTS xba_date DATE`);
